...
-- Table `GoAber`.`Group`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `GoAber`.`Group` ;

CREATE TABLE IF NOT EXISTS `GoAber`.`Group` (
...
CREATE TABLE IF NOT EXISTS `GoAber`.`User` (
  ...
  CONSTRAINT `group`
    FOREIGN KEY (`groupId`)
    REFERENCES `GoAber`.`Group` (`idGroup`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
